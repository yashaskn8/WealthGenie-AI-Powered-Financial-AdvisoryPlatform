...
| **MLOps Version Registry (Bundle-Native)** | [`ml-service/model/registry/`](ml-service/model/registry/) & [`ml-service/main.py`](ml-service/main.py) | SQLite for local development and MongoDB/GridFS for shared deployments; serving requires an explicit trusted-bundle bootstrap after Phase-3 migration, then startup reconciles and verifies the active complete bundle before readiness. Candidate evaluation evidence is immutable; promotion and rollback preload verified bundles and use generation-CAS activation. |
| **Distributed MongoDB State** | [`mongo_registry_store.py`](ml-service/model/registry/mongo_registry_store.py), [`mongo_vector_store.py`](ml-service/rag/vector_store/mongo_vector_store.py), and [`mongo_store.py`](ml-service/rag/lifecycle/mongo_store.py) | Shared model bundles, document revisions, corpus generations, and RAG chunks use MongoDB/GridFS; Mongo RAG ingestion and lifecycle transitions are transactional, with replicas refreshing against the shared corpus revision. |
...
- **Active Statutory Rule Version**: `getCurrentRegulatoryRuleVersion()` is the write-time authority exported by `server/services/taxEngine.js`; it resolves the India fiscal year on each call. At this baseline, the current date resolves to **FY2026-27 (AY2027-28)** and the identifier is `tax-policy-FY2026-27-v2`; the supported historical entry is `tax-policy-FY2025-26-v2`. The legacy `REGULATORY_RULE_VERSION` export is only a compatibility snapshot and is not used for new audit writes.
