...
export function buildReliabilityPromotionEvaluation({ scorecards = [], holdout = null, candidateReliabilityCoverageComplete = true } = {}) {
  ...
  const passed = Boolean(holdout?.sealed)
    && candidateReliabilityCoverageComplete === true
    && criticalFailures.length === 0
    && authorityDelta === 0
    && scorecards.every(card => card.hardGatePassed === true && card.passed === true);
  return Object.freeze({
    version: RELIABILITY_PROMOTION_GATE_VERSION,
    passed,
    holdoutSealed: Boolean(holdout?.sealed),
    candidateReliabilityCoverageComplete: candidateReliabilityCoverageComplete === true,
