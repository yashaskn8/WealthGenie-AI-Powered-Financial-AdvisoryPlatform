...
function hardGates({ trajectory = [], result = {}, caseDefinition = {} } = {}) {
  ...
  const financialAuthorityDelta = Number(result.financialAuthorityDelta ?? result.recommendationDelta ?? 0);
  ...
  return {
    ...
    financialAuthorityUnchanged: financialAuthorityDelta === 0,
...
export function buildCandidateScoreCard({ candidateId, partition, caseDefinition = {}, result = {}, trajectory = [] } = {}) {
  ...
  const hardGatePassed = Object.entries(gates)
    .filter(([key]) => !['forbiddenTools', 'unsupportedTools', 'financialAuthorityDelta'].includes(key))
    .every(([, passed]) => passed);
